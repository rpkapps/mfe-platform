...
import { ChevronDown, CircleHelp, LayoutGrid, Moon, Sun, UserRound } from 'lucide-react'
...
export function Shell({ boot }: { boot: ShellBoot }) {
  ...
  return (
    <AppShell className="flex h-full flex-col">
      ...
      <AppShellHeader>
        <AppShellBrand>
          <DropdownMenuTrigger isOpen={switcherOpen} onOpenChange={setSwitcherOpen}>
            <Button variant="ghost" size="sm" aria-label="Open app switcher" data-testid="shell.switcher">
              <LayoutGrid /> {currentApp?.title ?? shellConfig.product} <ChevronDown className="opacity-60" />
            </Button>
            <DropdownMenu className="min-w-64">
              <DropdownMenuLabel>Apps</DropdownMenuLabel>
              {apps.length === 0 ? <DropdownMenuItem isDisabled>No apps available</DropdownMenuItem>: null}
              {apps.map(app => (
                <DropdownMenuItem key={app.id} onAction={() => void runtime.navigate({ to: app.basePath })} textValue={app.title} data-testid={`shell.switcher.${app.id}`}>
                  <Icon svg={app.icon} className="[&_svg]:size-4" />
                  <span className="flex flex-col">
                    <span>{app.title}</span>
                    {app.description ? <span className="text-muted-foreground text-xs">{app.description}</span>: null}
                  </span>
                </DropdownMenuItem>
              ))}
            </DropdownMenu>
        ...
        <AppShellHeaderActions>
          ...
          <Button variant="ghost" size="icon-sm" aria-label="Toggle theme" onPress={() => setTheme(theme, theme.get().scheme === 'dark' ? 'light': 'dark')}>
            {theme.get().scheme === 'dark' ? <Sun />: <Moon />}
          </Button>
          <DropdownMenuTrigger>
            <Button variant="ghost" size="sm" aria-label="User menu" data-testid="shell.user">
              <UserRound /> {user?.user.displayName ?? 'Signed out'}
            </Button>
...
function visibleApps(release: Release, groups: readonly string[]): AppManifest[] {
  return Object.values(release.mfes).filter((m): m is AppManifest => m.kind === 'app').filter(m => requirementSatisfied(m.permissions, groups)).sort((a, b) => a.title.localeCompare(b.title))
}

/** Page-header actions are live registrations placed in 'page' plus the mounted app's static `to` actions. */
function PageActions({ runtime, currentApp, groups }: { runtime: ShellBoot['runtime']; currentApp: AppManifest | undefined; groups: readonly string[] }) {
  ...
  const items: Array<{ key: string; label: string; icon?: string; run: () => void; disabled: boolean; pending: boolean }> = [...live.map(r => registrationItem(r, statics.get(r.actionId), groups, runtime)),...navigation.map(a => ({ key: a.id, label: a.title, icon: a.icon, run: () => void runtime.actions.run({ id: a.id, initiator: 'user' }), disabled: false, pending: false })),
  ]
  if (items.length === 0) return null
  return (
    <div className="flex items-center gap-1" data-testid="shell.page-actions">
      {items.map(item => (
...
function registrationItem(r: RegistrationRecord, action: ManifestAction | undefined, groups: readonly string[], runtime: ShellBoot['runtime']) {
  ...
  return {
    ...
    label: r.target ? `${action?.title ?? r.action.title}`: (action?.title ?? r.action.title),
    icon: action?.icon,
...
function HelpMenu({ runtime, currentApp }: { runtime: ShellBoot['runtime']; currentApp: AppManifest | undefined }) {
  ...
  return (
    <DropdownMenuTrigger>
      <Button variant="ghost" size="icon-sm" aria-label="Help" data-testid="shell.help">
        <CircleHelp />
      </Button>
      <DropdownMenu placement="bottom end">
  ...
  return (
    <AlertDialog isOpen={pending !== null} onOpenChange={open => !open && finish(decision.current)} data-testid="shell.confirm">
      ...
      <AlertDialogFooter>
        ...
        <AlertDialogAction
          onPressStart={() => (decision.current = true)}
          onPress={() => finish(true)}
