...
export interface HttpClientOptions {
  /** The instance lifetime; every request is bounded by it (§36.1). */
  signal: AbortSignal
  fetch?: typeof fetch
  /** Origins that receive platform credentials and CSRF metadata. Same-origin is always approved. */
  apiOrigins: readonly string[]
  pageOrigin: string
  /** Called with a 401 so the shell can react (§31.1). The response is still returned to the caller. */
...
export function createHttpClient(options: HttpClientOptions): HttpClient {
  ...
  async function doFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    if (isApproved(url)) {
      ...
      // Outside the destination policy: no platform credentials or sensitive metadata (§36.1).
